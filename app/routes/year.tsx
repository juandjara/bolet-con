...
function Talks({ talks, formUrl }: { talks: Talk[]; formUrl?: string }) {
  return (
    <section>
      ...
      {formUrl ? (
        <p className="mx-auto mb-6 text-center">
          ...
          para enviar una nueva charla o sugerir una modificación a una ya
          existente
        </p>
      ) : null}
      {talks?.length ? (
        ...
        <p className="text-center py-3">No hay charlas todavia</p>
      )}
    </section>
  );
}
  ...
  return (
    <div className="mx-auto max-w-screen-md px-2">
      ...
      <main className="mb-12">
        <Talks talks={talks} formUrl={formUrl} />
      </main>
      <Map />
      <Footer />
    </div>
